...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    else if (huart->Instance == USART2_UX)                            /* 如果是串口2，进行串口2 MSP初始化 */
    {
        ...
        gpio_init_struct.Pin = USART2_RX_GPIO_PIN;               /* 串口RX脚  */ 
        gpio_init_struct.Alternate = USART2_RX_GPIO_AF;          /* 复用为USART2 */
        HAL_GPIO_Init(USART2_RX_GPIO_PORT, &gpio_init_struct);   /* 串口RX脚  */
        
#if USART2_EN_RX
        HAL_NVIC_EnableIRQ(USART2_UX_IRQn);                      /* 使能USART2中断通道 */
        HAL_NVIC_SetPriority(USART2_UX_IRQn, 3, 3);              /* 抢占优先级3，子优先级3 */
#endif
    }
}
    ...
    else if (huart->Instance == USART2_UX)                    /* 如果是串口2 */
    {
        if ((g_usart2_rx_sta & 0x8000) == 0)             /* 接收未完成 */
        {
            if (g_usart2_rx_sta & 0x4000)                /* 接收到了0x0d（即回车键） */
            {
                ...
                {
                    if((g_usart2_rx_sta & 0X3FFF)==0)   /* 数据长度为0 */
                    {
