...
TIM_HandleTypeDef b_timx_handle;         /* 定时器参数句柄 */
...
{
    btim_timx_int_init(10-1, 90-1);/* 100倍的系统时钟节拍，10us中断一次 */
}
...
{
    b_timx_handle.Instance = BTIM_TIMX_INT;                      /* 定时器x */
    b_timx_handle.Init.Prescaler = psc;                          /* 分频 */
    ...
    HAL_TIM_Base_Start_IT(&b_timx_handle);                       /* 使能定时器x和定时器更新中断 */
}
...
{
    HAL_TIM_IRQHandler(&b_timx_handle);  /* 定时器回调函数 */
}
